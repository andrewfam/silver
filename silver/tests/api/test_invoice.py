...
from silver.tests.api.specs.document_entry import spec_document_entry, check_document_entry_response
from silver.tests.api.specs.invoice import spec_invoice, check_invoice_response
...
def test_post_invoice_without_invoice_entries(authenticated_api_client, customer, provider):
    ...
    assert response.data == spec_invoice(invoice)
...
def test_get_invoices(authenticated_api_client, two_pages_of_invoices):
    assert len(two_pages_of_invoices) == settings.PAGE_SIZE * 2 > 2

    url = reverse('invoice-list')
    response = authenticated_api_client.get(url)

    assert response.status_code == status.HTTP_200_OK, response.data
    assert len(response.data) == settings.PAGE_SIZE
...
def test_get_invoice(authenticated_client, settings, issued_invoice):
    ...
    with patch('silver.utils.payments._get_jwt_token') as mocked_token:
        ...
        for show_pdf_storage_url, pdf_url in [
            ...
            settings.SILVER_SHOW_PDF_STORAGE_URL = show_pdf_storage_url

            response = authenticated_client.get(url, format='json')

            assert response.status_code == status.HTTP_200_OK, response.data
            check_invoice_response(invoice=invoice, response_data=response.data)
...
def test_add_single_invoice_entry(authenticated_api_client, invoice):
    ...
    check_document_entry_response(request_data, entry, response.data)
